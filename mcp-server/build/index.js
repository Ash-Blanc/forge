...
server.tool("read_excalidraw", "Reads an Excalidraw file and extracts the text elements to understand the diagrams content", {
...
// Tool: read_file
server.tool("read_file", "Read the content of a file", {
...
server.tool("search_codebase", "Search for a pattern in the codebase using grep", {
...
server.tool("github_dispatch_action", "Trigger a GitHub Actions workflow manually via workflow dispatch", {
...
server.tool("github_create_issue", "Create an issue on GitHub", {
    owner: z.string().describe("Repository owner"),
    repo: z.string().describe("Repository name"),
...
server.tool("github_add_comment", "Add a comment to a GitHub issue or pull request", {
    owner: z.string().describe("Repository owner"),
    repo: z.string().describe("Repository name"),
...
// Tool: save_idea
server.tool("save_idea", "Save an idea, thought, or note into the workspace (similar to Obsidian). Saves as a Markdown file with optional tags and references.", {
...
server.tool("get_ideas_graph", "Analyzes the saved ideas folder and returns a graph representation (nodes and edges) of how ideas are interconnected based on their links and tags.", {}, async () => {
    try {
        const ideasDir = path.join(process.cwd(), "ideas");
        try {
            await fs.access(ideasDir);
