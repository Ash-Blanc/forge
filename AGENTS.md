...
**Important**: Supabase keys must start with 'ey...' (JWT format), not 'sb_publishable_' (which are for Stripe).
...
If you encounter "Could not find the table 'public.analysis_sessions'" errors, you need to run the database migration:

1. **Manual Migration (Recommended)**:
   ```bash
   # Go to Supabase dashboard > Database > SQL Editor
   # Run the SQL from: supabase/migrations/20260303_complete_schema.sql
...
- **SSL Handshake Failed**: Check your Supabase URL and ensure it's using the correct project URL
- **Missing Tables**: Run the migration SQL in the Supabase dashboard
- **Invalid Keys**: Ensure SUPABASE_SERVICE_ROLE_KEY starts with 'ey...' (JWT format), not 'sb_publishable_'

### Supabase Configuration
- Always use service role key for migrations and admin operations
- Use anon key for client-side operations
- The database includes both old schema (papers, users) and new schema (analysis_sessions)
