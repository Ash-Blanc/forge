    ...
    for attempt in range(max_retries):
        try:
            ...
            if title_el is None or title_el.text is None:
                logger.error("Paper found but missing title. It may be unavailable.")
                return {}
            ...
            if e.code == 429:
                wait = 2 ** (attempt + 1)
                logger.warning(f"Rate limited by arXiv. Retrying in {wait}s...")
                time.sleep(wait)
                continue
            ...
            if attempt < max_retries - 1:
                time.sleep(1)
                continue
            logger.error(f"Failed to fetch from arXiv after {max_retries} attempts: {e}")
            return {}
