...
export default function PaperCard({ paper, selected, user, onSelect, onFollow, onClaim }: Props) {
    ...
    return (
        <article
            ...
            className={`
                card animate-fade-up p-3 sm:p-[14px_16px] cursor-pointer mb-2 
                transition-all duration-200 
                hover:scale-[1.01] hover:border-border-h
                ${selected 
                    ? "bg-elevated border-border-h shadow-[0_8px_26px_-16px_rgba(23,19,12,0.35)]" 
                    : "bg-surface border-border hover:shadow-[0_8px_20px_-16px_rgba(23,19,12,0.32)]"
                }
            `}
        >
            {/* Header */}
            <div className="flex justify-between items-start mb-2 sm:mb-[9px]">
                <div className="flex gap-2 items-center min-w-0 flex-1">
                    <Avatar name={paper.submittedBy.name} role={paper.submittedBy.role} size={26} />
                    <div className="min-w-0 flex-1">
                        <div className="text-text text-[0.7rem] sm:text-[0.72rem] font-mono font-semibold truncate">
            ...
            {/* Title */}
            <h3 className="text-[#2f281b] text-[0.8rem] sm:text-[0.875rem] font-semibold leading-[1.45] mb-2 line-clamp-2">
                {paper.title}
            </h3>

            {/* Opportunity */}
            {paper.opportunity && (
                <p className="text-muted text-[0.75rem] sm:text-[0.8rem] leading-[1.55] mb-2 sm:mb-[10px] italic line-clamp-2">
                    {paper.opportunity}
                </p>
            )}

            {/* Tags */}
            <div className="flex flex-wrap gap-1 mb-2 sm:mb-[11px]">
                {paper.tags?.slice(0, 4).map(t => <Tag key={t} text={t} />)}
            </div>

            {/* Footer */}
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-0">
                <div className="flex gap-2 sm:gap-[10px] items-center flex-wrap">
                    {/* Follow */}
                    <button
                        onClick={e => { e.stopPropagation(); onFollow(paper.id); }}
                        className={`
                            bg-transparent border-none cursor-pointer 
                            text-[0.68rem] sm:text-[0.7rem] font-mono 
                    ...
                    <span className="text-faint text-[0.68rem] sm:text-[0.7rem] font-mono">
                        {paper.comments.length} comment{paper.comments.length !== 1 ? "s" : ""}
                    </span>

                    {paper.claims.length > 0 && (
                        <span className="text-teal text-[0.68rem] sm:text-[0.7rem] font-mono">
                ...
                {claimant ? (
                    <div className="flex items-center gap-[5px]">
                        ...
                        <span className="text-muted text-[0.6rem] sm:text-[0.63rem] font-mono">
                    ...
                    <button
                        ...
                        className="
                            bg-teal-dim text-teal border-[1px] border-teal/15 
                            rounded-sm px-2 py-[2px] 
                            text-[0.65rem] sm:text-[0.67rem] 
