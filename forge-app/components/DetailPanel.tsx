...
export default function DetailPanel({ paper, user, onFollow, onClaim, onComment, onBuildUpdate, onRefresh }: Props) {
    ...
    const TAB_CLASS = (active: boolean) =>
        `px-2 sm:px-3 py-2 border-b-2 text-[0.63rem] sm:text-[0.67rem] font-mono uppercase tracking-[0.06em] transition-all duration-150 cursor-pointer ${active ? "text-text border-accent" : "text-muted border-transparent hover:text-text"
        }`;

    return (
        <div className="card flex flex-col h-full overflow-hidden">
            {/* Header */}
            <div className="p-3 sm:p-[14px_16px_12px] border-b border-border shrink-0 bg-surface">
                ...
                <h2 className="text-text text-[0.875rem] sm:text-[0.9375rem] font-bold leading-[1.4] mb-[5px]">
                    {paper.title}
                </h2>
                <p className="text-muted text-[0.65rem] sm:text-[0.69rem] font-mono">
            ...
            {/* Tabs */}
            <div className="flex bg-surface border-b border-border shrink-0 px-1 sm:px-2 sticky top-0 z-10">
            ...
            {/* Body */}
            <div className="scroll-y flex-1 p-3 sm:p-[14px_16px]">
                ...
                {tab === "analysis" && (
                    <div className="animate-fade-up">
                        ...
                        <div className="mb-4 sm:mb-[18px]">
                            ...
                            <p className="text-[#4e432e] text-[0.8rem] sm:text-[0.84rem] leading-[1.7]">
                                {typed}
                        ...
                        {/* Opportunity */}
                        {paper.opportunity && (
                            <div className="border-l-2 border-accent/40 pl-2 sm:pl-3 mb-3 sm:mb-4 bg-accent/5 p-2 sm:p-3 rounded-r-md">
                                <SectionLabel>Opportunity</SectionLabel>
                                <p className="text-[#2f281b] text-[0.8rem] sm:text-[0.84rem] leading-[1.6]">{paper.opportunity}</p>
                            </div>
                        )}

                        {/* 2-col details */}
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-[10px] mb-3 sm:mb-4">
                            {[
                                ...
                                <div key={label} className="bg-surface/50 rounded-md p-2 sm:p-[9px_11px] border border-border/50 backdrop-blur-sm">
                                    <SectionLabel>{label}</SectionLabel>
                                    <p className="text-muted text-[0.75rem] sm:text-[0.78rem] leading-[1.55] m-0">{val}</p>
                                </div>
                            ))}
                        </div>
                        ...
                        {paper.first90Days && paper.first90Days.length > 0 && (
                            <div className="mb-4 sm:mb-[18px]">
                                ...
                                {paper.first90Days.map((m, i) => (
                                    <div key={i} className="flex gap-2 sm:gap-[9px] mb-[7px] items-start">
                                        <span className="text-faint text-[0.65rem] sm:text-[0.69rem] font-mono mt-[3px] shrink-0">→</span>
                                        <p className="text-muted text-[0.75rem] sm:text-[0.8rem] leading-[1.5] m-0">{m}</p>
                                    </div>
                                ))}
                            </div>
                        )}
                        ...
                        {/* Action row */}
                        <div className="flex flex-col sm:flex-row gap-2 pt-2 sm:pt-1 border-t border-border/30 mt-3 sm:mt-4">
                            <button 
                                onClick={() => onFollow(paper.id)} 
                                className={`
                                    flex-1 rounded-md p-2 text-[0.7rem] sm:text-[0.72rem] 
                                    cursor-pointer font-mono transition-all duration-150
                                    ${following 
                                        ? "bg-accent-dim text-accent border border-accent/30" 
                                        : "bg-elevated text-muted border border-border hover:bg-surface hover:text-text"
                                    }
                                `}
                            >
                            ...
                            {!myClaim && paper.status === "open" && user?.role === "builder" && (
                                <button 
                                    onClick={() => onClaim(paper.id)} 
                                    className="flex-1 bg-teal-dim text-teal border border-teal/20 rounded-md p-2 text-[0.7rem] sm:text-[0.72rem] cursor-pointer font-mono hover:bg-teal/10 transition-colors"
                                >
                                    Claim & Build
                                </button>
                            )}
                            ...
                            {!myClaim && paper.status === "open" && user?.role !== "builder" && (
                                <div className="flex-1 bg-bg border border-border rounded-md p-2 text-center text-faint text-[0.67rem] sm:text-[0.69rem] font-mono flex items-center justify-center">
                                    Builders can claim
                                </div>
                            )}
                        </div>
                ...
                {tab === "discussion" && (
                    <div className="animate-fade-up">
                        ...
                        {paper.comments.map(c => (
                            <div key={c.id} className="flex gap-2 sm:gap-[10px] mb-3 sm:mb-4">
                                <Avatar name={c.user.name} role={c.user.role} size={26} />
                                <div className="flex-1 min-w-0">
                                    <div className="flex flex-wrap gap-[6px] items-center mb-1">
                                        <span className="text-text text-[0.72rem] sm:text-[0.75rem] font-mono font-semibold">{c.user.name}</span>
                                        <RoleBadge role={c.user.role} />
                                        <TimeAgo ts={c.createdAt} />
                                    </div>
                                    <p className="text-[#4e432e] text-[0.78rem] sm:text-[0.82rem] leading-[1.6] m-0 break-words">{c.text}</p>
                                </div>
                            </div>
                        ))}

                        {user && (
                            <div className="flex gap-2 sm:gap-[10px] mt-2 pt-3 border-t border-border mb-2">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <button 
                                        ...
                                        className="btn-primary mt-[6px] text-sm"
                                    >
                ...
                {tab === "build" && (
                    <div className="animate-fade-up">
                        {claimant ? (
                            <>
                                <div className="flex gap-2 sm:gap-[10px] items-center bg-teal-dim border border-teal/15 rounded-md p-2 sm:p-[10px_12px] mb-3 sm:mb-4">
                                    <Avatar name={claimant.user.name} role={claimant.user.role} size={28} />
                                    <div className="min-w-0">
                                        <div className="text-teal text-[0.72rem] sm:text-[0.75rem] font-mono font-bold">
                                            {claimant.user.name} is building this
                                        </div>
                                        <div className="text-muted text-[0.63rem] sm:text-[0.67rem] font-mono">
                                ...
                                {claimant.updates.length > 0 && (
                                    <div className="mb-3 sm:mb-4">
                                        ...
                                        {claimant.updates.map(u => (
                                            <div key={u.id} className="mb-3 pl-2 sm:pl-3 border-l-2 border-border/50">
                                                <p className="text-[#4e432e] text-[0.78rem] sm:text-[0.82rem] leading-[1.55] m-0 mb-[3px] break-words">
                                                    {u.text}
                                                </p>
                                ...
                                {myClaim && (
                                    <div className="pt-2 sm:pt-[10px] border-t border-border">
