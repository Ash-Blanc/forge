...
describe("POST /api/analyze-competitors", () => {
    beforeEach(() => {
        vi.clearAllMocks();
        vi.unstubAllGlobals();
    ...
    async function callRoute(body: Record<string, unknown>) {
        const { POST } = await import("@/app/api/analyze-competitors/route");
        const req = new Request("http://localhost/api/analyze-competitors", {
            method: "POST",
    ...
    it("returns 400 when ideaContext is missing", async () => {
        const res = await callRoute({});
        expect(res.status).toBe(400);
        const json = await res.json();
        expect(json.error).toMatch(/ideaContext required/i);
    });
    ...
    it("retries on 5xx upstream errors and eventually streams", async () => {
        ...
        const res = await callRoute({ ideaContext: "A great SaaS idea" });
    ...
    it("times out and falls back to error stream on AbortError", async () => {
        ...
        const res = await callRoute({ ideaContext: "Another idea" });
    ...
    it("returns 502 with error details immediately on 4xx without retrying", async () => {
        ...
        const res = await callRoute({ ideaContext: "Fail me" });
